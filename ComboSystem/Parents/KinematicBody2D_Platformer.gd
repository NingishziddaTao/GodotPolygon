...
export(Array, Resource) var ray_exceptions = [self]
var player
export(Array, Resource) var resource_method_array
var afflicted

func resource_method(method:String, body, arg1=null, arg2=null):
    for i in resource_method_array:
        if i != null:
            if i.has_method(method):
                i.call(method, body, arg1, arg2)

func _ready() -> void:
    ray_exceptions.append(Hud.player)
    x_ray_check = (yWidth + yWidth) / steps
...
func _draw() -> void:
    ...
    for i in range(-yWidth, yWidth, steps):
        ...
        y_result = space_state.intersect_ray(global_position+y_ray_position,
                                               y_ray_position+global_position+y_ray_cast_to, ray_exceptions)
        if y_result:
        ...
        # The rays wil be casted fram left to right this check is nessesary
        elif y_array == []:
    ...
    for i in range(-xWidth, xWidth, steps):
        ...
        x_result = space_state.intersect_ray(global_position+x_ray_position,
                                               x_ray_position+global_position+x_ray_cast_to, ray_exceptions)
        if x_result:
